...
def errors(ex, message=None) -> dict:
    """
    ...
    return {
        ...
        "error_message": message or ex.args[0],
    }
...
class DAO:
    ...
    @classmethod
    async def search_by_fields(cls, model, data_dict: dict, session):
        obj = await session.execute(select(model).filter_by(**data_dict))
        return obj.scalar_one_or_none()

    @classmethod
    async def add_object(cls, model, data_dict, session):
    ...
    @classmethod
    async def delete_object(cls, obj, session):
        await session.delete(obj)
        await session.commit()

    @classmethod
    async def change_object(cls, obj, data_dict, session):
    ...
    @classmethod
    async def seed_data_statement(cls, session):
        """Предоставляет команды создания начальных данных для вновь созданных таблиц базы данных."""
        user_data = [
    ...
    @classmethod
    async def get_user(cls, field, field_value, session) -> User:
        """Возвращает user, присоединяя к нему данные от таблицы user_follow"""
    ...
    @classmethod
    async def get_profile(cls, field, field_value, response, session) -> dict:
        """Возвращает данные профиля пользователя"""
        user = await DAO.get_user(field, field_value, session)
    ...
    @classmethod
    async def follow_control(cls, id, api_key, method_name, session) -> dict:
        """Управление подпиской"""
        user_follower = await DAO.get_user(
    ...
    @classmethod
    async def like_control(cls, like_id, api_key, method_name, session) -> dict:
        """Управление лайками"""
        # Получаем твит
        tweet_object = await session.execute(
            select(Tweet)
    ...
    @classmethod
    async def get_followed_ids(cls, api_key, session) -> dict:
        """Возвращает словарь со списком id зафолловленных и id текущего пользователя"""
        user_object = await session.execute(
            select(User)
            .options(selectinload(User.following))
    ...
    @classmethod
    async def get_last_tweet_id(cls, user_id, session) -> int | None | dict:
        """Возвращает id последнего твита пользователя"""
    ...
    @classmethod
    async def get_tweets_ordered_from_like(cls, following_ids, session) -> list | dict:
        """Возвращает список твитов своих и зафолловленных, отсортированных по популярности"""
            ...
            (
                ...
                | {
                    "likes": [
                        ...
                        for user in tweet.users_who_liked
                    ]
                }
    ...
    @classmethod
    async def delete_tweet(cls, api_key, id, session):
        tweet_object = await session.execute(
            select(Tweet)
            .join(User)
        ...
        tweet = tweet_object.scalar_one()
        await DAO.delete_object(tweet, session)
